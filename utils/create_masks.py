"""
Original file from: https://github.com/DIUx-xView/xView2_first_place
...
if __name__ == '__main__':
    ...
    all_files = []
    for d in train_dirs:
        masks_path = path.join(d, masks_dir)
        makedirs(masks_path, exist_ok=True)
        print(f"Created masks directory: {masks_path}")
        
        labels_path = path.join(d, 'labels')
        if not path.exists(labels_path):
            print(f"Warning: Labels directory not found: {labels_path}")
            continue
            
        for f in sorted(listdir(labels_path)):
            if '_pre_disaster.json' in f:
                json_path = path.join(labels_path, f)
                all_files.append(json_path)
                print(f"Found label file: {json_path}")

    print(f"\nTotal files to process: {len(all_files)}")

    if len(all_files) == 0:
        print("No label files found! Check your data directory structure.")
    else:
        # Process sequentially for better error reporting
        for f in all_files:
            process_image(f)
