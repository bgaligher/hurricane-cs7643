...
Edits:
 - Added class DirSplitSLDataModule(pl.LightningDataModule) which does not split data by disasters
 and holds test and final holdout set
'''
...
class DirSplitSLDataModule(pl.LightningDataModule):
    """
    Simpler datamodule without disaster-based splitting
      - train_dirs: list of directories used for training
      - val_dirs:   list of directories used for validation
      - test_dirs:  list of directories used for test/eval
    """
    ...
    def __init__(self,
                 ...
                 val_dirs=None,
                 test_dirs=None,
                 labeled_transforms: torchvision.transforms.Compose = None,
                 labeled_batch_size: int = 8,
                 num_workers: int = 4,
                 **kwargs):
        super().__init__(**kwargs)
        ...
        self.train_files = self._collect_files(self.train_dirs)
        self.val_files = self._collect_files(self.val_dirs)
        self.test_files = self._collect_files(self.test_dirs)
    ...
    def setup(self, stage: str):
        ...
        if stage == "test" or stage == "predict" or stage is None:
            if self.test_files:
                ...
                self.pred_dataset = self.test_dataset
