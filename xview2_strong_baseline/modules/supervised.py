...
Edits:
 - Added on_test_epoch_end function into SLMOdule to calculate f1 harmonic mean
'''
...
class SLModule(LightningModule):
    ...
    # NEW: to calculate harmonic mean F1 score of damage classification
    def on_test_epoch_end(self):
        dmg_f1_list = [self.test_metric[i].compute() for i in range(1, 5)]
        dmg_f1 = torch.stack(dmg_f1_list)
        eps = 1e-7
        dmg_f1_harm = 4.0 / torch.sum(1.0 / (dmg_f1 + eps))
        self.log("test_damage_F1_harmonic", dmg_f1_harm, prog_bar=True)
